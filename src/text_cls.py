...
class Trainer:
    ...
    def train_test_loop(self, mode='train', epoch=1000):
        ...
        for step, ex in tqdm(enumerate(self.data_loaders[mode])):
            ...
            # Update
            if mode == 'train' and not epoch >= self.args.epochs:
        ...
        if not self.args.store:
            ...
            with open(f'{self.args.dataset}_pred_spans_{mode}.json', 'w') as fid:
...
class Joint3DDataset(Dataset):
    ...
    def load_scanrefer_annos(self):
        ...
        _path = self.data_path + 'ScanRefer_filtered'
        split = self.split
