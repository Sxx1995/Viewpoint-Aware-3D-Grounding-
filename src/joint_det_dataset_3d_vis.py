...
class Joint3DDataset(Dataset):
    ...
    def __getitem__(self, index, matrix, center, pred):
        ...
        scan = scannet_loader([anno['scan_id'], '/nfs/hpc/sw/xiangxi/scannet/DATA/scans/'])
        ...
        self._visualize_scene(anno, point_cloud, og_color, all_bboxes, matrix, center, pred)
    ...
    def _visualize_scene(self, anno, point_cloud, og_color, all_bboxes, rotate_matrix, predict_center, pred_bbox):
        ...
        rotate_matrix2 = np.array([[-1, 0, 0], [0, -1, 0], [0, 0, 1]])
        #point_cloud[:, :3] = np.matmul(np.matmul(point_cloud[:, :3] - predict_center[np.newaxis, :], rotate_matrix), rotate_matrix2)
        point_cloud[:, :3] = np.matmul(point_cloud[:, :3] - predict_center[np.newaxis, :], rotate_matrix)
        pred_b = np.zeros((8, 3))
        print(pred_bbox.shape, pred_b.shape)
        pred_bbox = pred_bbox.cpu().detach().numpy()
        pred_b[0, 0] = pred_bbox[0]
        pred_b[0, 1] = pred_bbox[1]
        pred_b[0, 2] = pred_bbox[2]

        pred_b[1, 0] = pred_bbox[0+3]
        pred_b[1, 1] = pred_bbox[1]
        pred_b[1, 2] = pred_bbox[2]

        pred_b[2, 0] = pred_bbox[0+3]
        pred_b[2, 1] = pred_bbox[1+3]
        pred_b[2, 2] = pred_bbox[2]

        pred_b[3, 0] = pred_bbox[0]
        pred_b[3, 1] = pred_bbox[1+3]
        pred_b[3, 2] = pred_bbox[2]

        pred_b[4, 0] = pred_bbox[0]
        pred_b[4, 1] = pred_bbox[1+3]
        pred_b[4, 2] = pred_bbox[2+3]

        pred_b[5, 0] = pred_bbox[0+3]
        pred_b[5, 1] = pred_bbox[1+3]
        pred_b[5, 2] = pred_bbox[2+3]

        pred_b[6, 0] = pred_bbox[0+3]
        pred_b[6, 1] = pred_bbox[1]
        pred_b[6, 2] = pred_bbox[2+3]

        pred_b[7, 0] = pred_bbox[0]
        pred_b[7, 1] = pred_bbox[1]
        pred_b[7, 2] = pred_bbox[2+3]
        #point_cloud[:, :3] = np.matmul(point_cloud[:, :3] - predict_center[np.newaxis, :], rotate_matrix)
        o = []
        for i in tqdm(point_cloud):
            if i[2] <= 2.0:
                o.append(i)
        point_cloud = np.stack(o)
        centers = np.random.rand(5000,3) - 0.5
        centers2 = np.random.rand(5000,3) - 0.5
        centers3 = np.random.rand(5000,3) - 0.5
        centers_dis = ((centers**2).sum(-1)**0.5)[:, np.newaxis] 
        centers_dis2 = ((centers2**2).sum(-1)**0.5)[:, np.newaxis] 
        centers_dis3 = ((centers3**2).sum(-1)**0.5)[:, np.newaxis] 
        centers = centers / centers_dis * 0.15
        centers2 = centers2 / centers_dis2 * 0.15
        centers3 = centers3 / centers_dis3 * 0.15
        center_color = np.ones(centers.shape) * 256
        center_color[:, 1:] = 0
        center_color2 = np.ones(centers.shape) * 256
        center_color2[:, 0] = 0
        center_color2[:, 1] = 0
        center_color3 = np.ones(centers.shape) * 256
        center_color3[:, 0] = 0
        center_color3[:, 2] = 0
        centers2[:, 0] += 1
        centers3[:, 1] += 1
        l = np.arange(0, 1, 0.0002)
        Line_x = np.zeros((5000, 3))
        Line_x1 = np.zeros((5000, 3))
        Line_x2 = np.zeros((5000, 3))
        la = np.arange(1, 0.8, -0.2/5000)
        Line_x1[:, 0] = la
        Line_x1[:, 1] = 1 - la
        Line_x2[:, 0] = la
        Line_x2[:, 1] = la - 1
        Line_x[:, 0] = l
        Line_y = np.zeros((5000, 3))
        Line_y1 = np.zeros((5000, 3))
        Line_y2 = np.zeros((5000, 3))
        Line_y[:, 1] = l
        Line_y1[:, 1] = la
        Line_y1[:, 0] = 1 - la
        Line_y2[:, 1] = la
        Line_y2[:, 0] = la - 1
        Line_x = np.concatenate([Line_x, center_color2], axis = 1)
        Line_y = np.concatenate([Line_y, center_color3], axis = 1)
        Line_x1 = np.concatenate([Line_x1, center_color2], axis = 1)
        Line_y1 = np.concatenate([Line_y1, center_color3], axis = 1)
        Line_x2 = np.concatenate([Line_x2, center_color2], axis = 1)
        Line_y2 = np.concatenate([Line_y2, center_color3], axis = 1)
        #centers2 = np.concatenate([centers2, center_color2], axis = 1)
        #centers3 = np.concatenate([centers3, center_color3], axis = 1)
        centers = np.concatenate([centers, center_color], axis = 1)
        z = np.concatenate([centers, Line_x, Line_y,Line_x1, Line_y1,Line_x2, Line_y2], axis = 0)
        z[:, 2] = 3
        point_cloud = np.concatenate([point_cloud, z], axis = 0)

        all_boxes_points = box2points(all_bboxes[..., :6])

        #all_boxes_points = np.matmul(np.matmul(all_boxes_points - predict_center[np.newaxis, :], rotate_matrix), rotate_matrix2)
        #pred_b = np.matmul(np.matmul(pred_b - predict_center[np.newaxis, :], rotate_matrix), rotate_matrix2)
        pred_b = np.matmul(pred_b - predict_center[np.newaxis, :], rotate_matrix)
        all_boxes_points = np.matmul(all_boxes_points - predict_center[np.newaxis, :], rotate_matrix)
        target_box = all_boxes_points[target_id]
        #target_box = np.matmul(np.matmul(target_box - predict_center[np.newaxis, :], rotate_matrix), rotate_matrix2)
        print(target_box, pred_b)
            ...
            "ground_truth_point_scene": wandb.Object3D({
                ...
                "boxes": np.array(
                    ...
                    + [  # other
                        ...
                        for i, c in enumerate(all_boxes_points[:len(all_boxes_points)//2])
                        if i not in (
                            [target_id]
                            + anchor_ids.tolist()
