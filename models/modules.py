...
class ClsAgnosticPredictHead(nn.Module):
    ...
    def forward(self, features, base_xyz, end_points, prefix='',rotate_matrix = None ,predict_viewpoint_center = None, pre_rotate_matrix = None , pre_predict_viewpoint_center = None):
        """
        Args:
        ...
        end_points[f'{prefix}pred_size'] = F.relu(pred_size)
        if rotate_matrix is not None:
            center = torch.bmm(center, torch.linalg.inv(rotate_matrix)) + predict_viewpoint_center.unsqueeze(1)
        if pre_rotate_matrix is not None:
            center = torch.bmm(center, torch.linalg.inv(pre_rotate_matrix)) + pre_predict_viewpoint_center.unsqueeze(1)
